//@version=5
strategy("MSB + Order Block Strategy",
     overlay=true,
     initial_capital=100,
...
if ta.change(market) != 0
    if market == 1
        ...
        label.new(int(math.avg(h1i, l0i)), h1, "MSB-long", color=color.new(color.black, 100), style=label.style_label_down, textcolor=color.green, size=size.small)
    ...
    if market == -1
        ...
        label.new(int(math.avg(l1i, h0i)), l1, "MSB-short", color=color.new(color.black, 100), style=label.style_label_up, textcolor=color.red, size=size.small)
...
// === GET LAST OB (ENTRY & TARGET) ===
bu_ob = array.size(bu_ob_boxes) > 0 ? array.get(bu_ob_boxes, array.size(bu_ob_boxes)-1) : na
be_ob = array.size(be_ob_boxes) > 0 ? array.get(be_ob_boxes, array.size(be_ob_boxes)-1) : na

// Bullish OB (Buy OB)
bu_top = not na(bu_ob) ? box.get_top(bu_ob) : na
bu_bot = not na(bu_ob) ? box.get_bottom(bu_ob) : na

// Bearish OB (Sell OB)
be_top = not na(be_ob) ? box.get_top(be_ob) : na
be_bot = not na(be_ob) ? box.get_bottom(be_ob) : na

// === ENTRY CONDITIONS (price inside OB) ===
longCond  = isBull and not na(bu_ob) and close < bu_top and close > bu_bot
shortCond = isBear and not na(be_ob) and close > be_bot and close < be_top

// === EXECUTION ===
if longCond and strategy.position_size == 0 and not na(be_ob)
    sl = bu_bot
    tp = be_bot   // TP = nearest bearish OB
    strategy.entry("LONG", strategy.long)
    strategy.exit("LONG EXIT", "LONG", stop=sl, limit=tp)

if shortCond and strategy.position_size == 0 and not na(bu_ob)
    sl = be_top
    tp = bu_top   // TP = nearest bullish OB
